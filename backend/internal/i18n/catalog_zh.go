...
func init() {
	register(LocaleZH, map[string]string{
		...
		// Agent
		MsgAgentNotFound:       "未找到 Agent：%s",
		MsgCannotDeleteDefault: "无法删除默认 Agent",
		...
		// Channel instances
		MsgInvalidChannelType: "Channel 类型无效",
		...
		MsgUserIDHeader:          "需要 X-Qorven-User-Id 请求头",
		...
		MsgNoAccess:              "无权访问此%s",
		MsgAlreadySummoning:      "Agent 正在被召唤中",
		MsgSummoningUnavailable:  "召唤功能不可用",
		MsgNoDescription:         "Agent 没有可供重新召唤的描述",
		MsgInvalidPath:           "路径无效",

		// Scheduler
		MsgQueueFull:    "Session 队列已满",
		...
		// Agent links
		MsgLinksNotConfigured:   "Agent 链接未配置",
		MsgInvalidDirection:     "方向必须是 outbound、inbound 或 bidirectional",
		MsgSourceTargetSame:     "源和目标必须是不同的 Agent",
		MsgCannotDelegateOpen:   "无法委派给开放型 Agent — 只有预定义 Agent 才能作为委派目标",
		MsgNoUpdatesProvided:    "未提供更新内容",
		MsgInvalidLinkStatus:    "状态必须是 active 或 disabled",

		// Teams
		MsgTeamsNotConfigured:   "团队未配置",
		MsgAgentIsTeamLead:      "该 Agent 已是团队负责人",
		MsgCannotRemoveTeamLead: "无法移除团队负责人",

		// Channels
		MsgCannotDeleteDefaultInst: "无法删除默认 Channel 实例",
		MsgCannotRemoveLastWriter:  "无法移除最后一个文件管理员",

		// Skills
		MsgSkillsUpdateNotSupported: "基于文件的 Skill 不支持 skills.update",
		MsgCannotResolveSkillID:     "无法解析基于文件的 Skill ID",
		...
		// Storage / File
		MsgCannotDeleteSkillsDir: "无法删除 Skill 目录",
