	...
	return []guardPattern{
		{
			...
			pattern: regexp.MustCompile(`(?i)(ignore|disregard|forget|override|bypass)\s+(all\s+)?(your\s+|my\s+|previous|prior|above|earlier|preceding)?\s*(instructions?|rules?|prompts?|directives?|guidelines?|constraints?)`),
		},
		{
			name:    "role_override",
