...
// ToolPolicySpec defines tool access policy for an agent.
type ToolPolicySpec struct {
	AlsoAllow []string `json:"also_allow"`
}

// MemberRequestConfig controls whether team members can request help from each other.
type MemberRequestConfig struct {
	Enabled bool `json:"enabled"`
}

// ParseMemberRequestConfig extracts member request configuration from team settings.
func ParseMemberRequestConfig(settings map[string]any) MemberRequestConfig {
	cfg := MemberRequestConfig{Enabled: true}
	if settings == nil {
		return cfg
	}
	if v, ok := settings["member_requests"]; ok {
...
func buildTeamMD(team *Team, members []TeamMember, selfID uuid.UUID) string {
	...
	if selfRole == TeamRoleLead {
		var reviewers []TeamMember
		for _, m := range members {
			if m.Role == TeamRoleReviewer {
...
func agentToolPolicyForTeam(policy *ToolPolicySpec, _ bool) *ToolPolicySpec {
	return policy
}
...
func agentToolPolicyWithMCP(policy *ToolPolicySpec, hasMCP bool) *ToolPolicySpec {
	if !hasMCP {
		return policy
	}
	if policy == nil {
...
func agentToolPolicyWithWorkspace(policy *ToolPolicySpec, hasTeam bool) *ToolPolicySpec {
	if !hasTeam {
		return policy
	}
	if policy == nil {
