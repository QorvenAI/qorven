...
func (gw *Gateway) registerV1Routes(parent chi.Router) {
	parent.Route("/v1", func(r chi.Router) {
		...
		r.Get("/budgets/effective", gw.handleEffectiveBudget)
		r.Get("/budgets/proposals", gw.handleListProposals)
		...
		r.Post("/user/api-keys", gw.handleCreateAPIKey)
		r.Delete("/user/api-keys/{id}", gw.handleRevokeAPIKey)
		r.Post("/user/password", gw.handleChangePassword)
		...
		r.Get("/social/approvals", gw.handleListSocialPendingApprovals)
		r.Post("/social/posts/{id}/approve", gw.handleApproveSocialPost)
		r.Post("/social/posts/{id}/reject", gw.handleRejectSocialPost)
		r.Get("/social/campaigns", gw.handleListCampaigns)
		r.Post("/social/campaigns", gw.handleCreateCampaign)
		r.Get("/social/campaigns/{id}", gw.handleGetCampaign)
		r.Put("/social/campaigns/{id}/status", gw.handleSetCampaignStatus)
		r.Post("/social/campaigns/{id}/delegate", gw.handleDelegateCampaign)
		...
		r.Get("/mail/search", gw.handleMailSearch)
		r.Get("/mail/thread/{thread_id}", gw.handleGetMailThread)
		r.Post("/mail/send", gw.handleSendMail)
		r.Post("/mail/bulk", gw.handleMailBulk)
		r.Get("/mail/drafts", gw.handleListMailDrafts)
		r.Post("/mail/drafts", gw.handleSaveMailDraft)
		r.Put("/mail/drafts/{id}", gw.handleUpdateMailDraft)
		r.Delete("/mail/drafts/{id}", gw.handleDeleteMailDraft)
		r.Get("/mail/{id}", gw.handleGetMail)
		r.Delete("/mail/{id}", gw.handleMailDelete)
		r.Put("/mail/{id}/read", gw.handleMailRead)
		r.Put("/mail/{id}/star", gw.handleMailStar)
		r.Post("/mail/{id}/archive", gw.handleMailArchive)
		r.Post("/mail/{id}/move", gw.handleMailMove)
		r.Get("/mail/{id}/attachments/{name}", gw.handleMailAttachment)
		...
		r.Get("/tunnel/status", gw.handleTunnelStatus)
		r.Post("/tunnel/enable", gw.handleTunnelEnable)
		r.Post("/tunnel/disable", gw.handleTunnelDisable)
