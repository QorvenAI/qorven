...
// createdBy identifies the persona that authored this revision (e.g. "cto", "cfo").
func (gw *Gateway) upsertArtifactRevision(ctx context.Context, briefID, typ, contentMD, createdBy string) (*ProjectArtifact, error) {
	...
	err := gw.db.Pool.QueryRow(ctx,
		`INSERT INTO project_artifacts (tenant_id, brief_id, type, version, content_md, status, created_by)
		 ...
		 RETURNING id, brief_id, type, version, content_md, status, repo_committed,
		           created_by, approved_by, approved_at, created_at`,
		defaultTenant, briefID, typ, nextArtifactVersion(prevV), contentMD, createdBy).
		Scan(&a.ID, &a.BriefID, &a.Type, &a.Version, &a.ContentMD, &a.Status,
			&a.RepoCommitted, &a.CreatedBy, &a.ApprovedBy, &a.ApprovedAt, &a.CreatedAt)
	if err != nil { return nil, err }
	return &a, nil
...
// handleGenerateArtifact drafts an artifact (prd|spec|design|resource_plan).
...
func (gw *Gateway) handleGenerateArtifact(w http.ResponseWriter, r *http.Request) {
	...
	if typ != "prd" && typ != "spec" && typ != "design" && typ != "resource_plan" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "type must be prd, spec, design, or resource_plan"}); return
	}
	brief, err := gw.readBrief(r.Context(), id)
	...
	// resource_plan is generated by the CFO cost engine (deterministic), not via LLM.
	if typ == "resource_plan" {
		md, err := gw.generateResourcePlan(r.Context(), brief)
		if err != nil { writeJSON(w, http.StatusInternalServerError, map[string]string{"error": sanitizeError(err)}); return }
		art, err := gw.upsertArtifactRevision(r.Context(), id, typ, md, "cfo")
		if err != nil { writeJSON(w, http.StatusInternalServerError, map[string]string{"error": sanitizeError(err)}); return }
	...
	art, err := gw.upsertArtifactRevision(r.Context(), id, typ, md, "cto")
	if err != nil { writeJSON(w, http.StatusInternalServerError, map[string]string{"error": sanitizeError(err)}); return }
	_, _ = gw.db.Pool.Exec(r.Context(), `UPDATE project_briefs SET stage=$2, updated_at=now() WHERE id=$1`, id, typ)
	writeJSON(w, http.StatusOK, art)
}

// generateResourcePlan builds the CFO resource plan for a brief using the
// cost engine. It is deterministic (no LLM call) and executes synchronously.
func (gw *Gateway) generateResourcePlan(ctx context.Context, b *ProjectBrief) (string, error) {
	roles := gw.cfoProposeRoles(b)
	...
	if gw.db != nil && gw.db.Pool != nil {
		enabled = providers.NewKeyPoolStore(gw.db.Pool, gw.cfg.Auth.EncryptionKey).EnabledModelIDs(ctx, defaultTenant)
	}

	cat, _ := providers.LoadCatalog()

	snap := gatewayllm.GetPricingSnapshot()
	rates := make(map[string]ModelPricingLite, len(snap))
	for id, mp := range snap {
		rates[id] = ModelPricingLite{Input: mp.InputPer1M, Output: mp.OutputPer1M}
	}

	pick := func(role, tier string) (string, string) {
		m := providers.SelectModelForHire(cat, role, tier, availProviders, enabled)
		if m == "" {
			return "auto", ""
		}
		provName := ""
		if p := gw.providerReg.ProviderForModel(m); p != nil {
			provName = p.Name()
		}
		return m, provName
	}

	budgetUSD := float64(b.BudgetCents) / 100.0
	plan := buildResourcePlan(roles, b.Quality, b.Timeline, budgetUSD, pick, rates)
	if len(enabled) == 0 {
		plan.Notes = append(plan.Notes, "no models enabled — curate providers in Settings; cannot guarantee execution")
	}
	return plan.ToMarkdown(), nil
}

// cfoProposeRoles derives the set of agent roles required for a project brief
// by delegating to the team planner and extracting the role names.
func (gw *Gateway) cfoProposeRoles(b *ProjectBrief) []string {
	tp := PlanTeam(b.Idea, b.Stack, b.Quality, b.BudgetCents)
	roles := make([]string, 0, len(tp.Agents))
