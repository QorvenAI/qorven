...
// RequiredSchemaVersion is the current required database schema version.
...
func FormatError(s *SchemaStatus) string {
	...
	if s.CurrentVersion > s.RequiredVersion {
		return fmt.Sprintf(
			"Database schema (v%d) is newer than this binary (requires v%d).\n"+
				"You may be running an older version of qorven.\n\n"+
				"  Fix: upgrade your qorven binary to the latest version.\n",
	...
	return fmt.Sprintf(
		"Database schema is outdated: current v%d, required v%d.\n\n"+
			"  Run:  ./qorven upgrade\n"+
			"  Or:   ./qorven migrate up   (SQL-only, no data hooks)\n\n"+
			"  Docker/CI: set QORVEN_AUTO_UPGRADE=true to upgrade automatically on startup.\n",
		s.CurrentVersion, s.RequiredVersion,
	)
}
