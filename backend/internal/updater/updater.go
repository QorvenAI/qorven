...
import (
	...
	"github.com/qorvenai/qorven/internal/version"
)

const (
	githubRepo = "QorvenAI/qorven"
	tagPrefix  = "qorven-v"
...
// CheckForUpdate queries GitHub Releases for a newer qorven-v* release.
...
func DownloadAndApply(info *UpdateInfo, appPath string) error {
	...
	tmpDir, err := os.MkdirTemp("", "qorven-update-*")
	if err != nil {
...
func applyMacOS(r io.Reader, tmpDir, appPath string) error {
	...
	// Find extracted .app
	newApp := filepath.Join(tmpDir, "qorven-lite.app")
...
func applyWindows(r io.Reader, tmpDir, exePath string) error {
	...
	// Extract to temp
	newExe := filepath.Join(tmpDir, "qorven-lite.exe")
...
func ResolveAppPath() (string, error) {
	...
	switch runtime.GOOS {
	case "darwin":
		// exe: /path/to/Qorven Lite.app/Contents/MacOS/qorven-lite
		// app: /path/to/Qorven Lite.app
