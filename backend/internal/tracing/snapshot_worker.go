...
func NewSnapshotWorker(pool *pgxpool.Pool, snapshots store.SnapshotStore) *SnapshotWorker {
...
func (w *SnapshotWorker) Backfill(ctx context.Context) (int, error) {
	...
	startHour := earliest.UTC().Truncate(time.Hour)
	if latest != nil {
		startHour = latest.Add(time.Hour)
	}
...
func queryTraceAggregates(ctx context.Context, pool *pgxpool.Pool, from, to time.Time) ([]traceAggregate, error) {
...
func querySpanAggregates(ctx context.Context, pool *pgxpool.Pool, from, to time.Time) ([]spanAggregate, error) {
	rows, err := pool.Query(ctx, `
		SELECT
			t.agent_id,
...
func queryMemoryCounts(ctx context.Context, pool *pgxpool.Pool) (map[uuid.UUID]agentMemoryCounts, error) {
	result := make(map[uuid.UUID]agentMemoryCounts)

	// Document counts
	rows, err := pool.Query(ctx, `SELECT agent_id, COUNT(*) FROM memory_documents GROUP BY agent_id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	// Chunk counts
	rows2, err := pool.Query(ctx, `SELECT agent_id, COUNT(*) FROM memory_chunks GROUP BY agent_id`)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
...
func queryKGCounts(ctx context.Context, pool *pgxpool.Pool) (map[uuid.UUID]agentKGCounts, error) {
	result := make(map[uuid.UUID]agentKGCounts)

	rows, err := pool.Query(ctx, `SELECT agent_id, COUNT(*) FROM kg_entities GROUP BY agent_id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	rows2, err := pool.Query(ctx, `SELECT agent_id, COUNT(*) FROM kg_relations GROUP BY agent_id`)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
