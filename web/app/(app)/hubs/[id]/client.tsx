  ...
  if (isUser) {
    return (
      <div className="flex justify-end py-1">
        <div className={cn(
          'max-w-[75%] rounded-2xl rounded-tr-sm bg-primary/10 px-3.5 py-2.5 text-2sm leading-relaxed',
          msgTypeStyle
        )}>
          {msg.content}
        </div>
  ...
  return (
    <div className="flex items-start gap-2.5 py-1.5">
      {member?.avatar ? (
        ...
        <div
          className={`flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br text-2xs font-bold text-white shrink-0 mt-0.5 ${gradient}`}
        >
          {(name[0] ?? '?').toUpperCase()}
        </div>
      )}
      <div className="flex-1 min-w-0">
        ...
        <div className={cn(
          'text-2sm leading-relaxed text-foreground whitespace-pre-wrap break-words',
          msgTypeStyle
        )}>
          {msg.content}
        </div>
  ...
  return (
    <div className="flex h-[calc(100vh-var(--header-height)-1px)] flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-2 shrink-0">
        {showBack && (
          <Link href="/hubs" className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-accent">
        ...
        {workOpen ? (
          <aside className="hidden lg:flex w-[300px] shrink-0 flex-col border-l border-border overflow-hidden">
            <div className="flex items-center gap-2 border-b border-border px-3 py-2 shrink-0">
              ...
              <span className="text-2xs text-muted-foreground">{work.length}</span>
              ...
              {work.map(wi => (
                ...
                >
                  ...
                  <div className="mt-1 flex items-center gap-1.5">
                    <span className={cn('rounded px-1.5 py-0.5 text-2xs font-medium', STATUS_COLOR[wi.status] ?? 'bg-muted text-muted-foreground')}>
                      {wi.status.replaceAll('_', ' ')}
                    </span>
                    {(wi.owner_key || wi.owner_name) && (
                      <span className="text-2xs text-muted-foreground truncate">@{wi.owner_key || wi.owner_name}</span>
                    )}
                    {wi.updated_at && (
                      <span className="text-2xs text-muted-foreground ml-auto">{new Date(wi.updated_at).toLocaleDateString()}</span>
                    )}
                  </div>
                </button>
              ))}
          ...
          >
            ...
            <span className="text-2xs">{work.length}</span>
          </button>
        )}
      </div>
          ...
          <div className="h-full w-[380px] bg-background border-l border-border flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-2 border-b border-border px-4 py-3 shrink-0">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex items-center gap-1.5">
                  <span className={cn('rounded px-1.5 py-0.5 text-2xs font-medium', STATUS_COLOR[selectedWork.item.status] ?? 'bg-muted text-muted-foreground')}>
                    {selectedWork.item.status.replaceAll('_', ' ')}
                  </span>
                  {(selectedWork.item.owner_key || selectedWork.item.owner_name) && (
                    <span className="text-2xs text-muted-foreground">@{selectedWork.item.owner_key || selectedWork.item.owner_name}</span>
                  )}
                </div>
              </div>
