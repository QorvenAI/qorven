...
export const channelFormSchemas: Record<ChannelType, ChannelSchema> = {
  ...
  sms: {
    ...
    setupSteps: [
      ...
      'Go to Account → Account Info; copy your Account SID and Auth Token',
    ...
    fields: [
      ...
      { key: 'account_sid', label: 'Account SID', type: 'text', required: true,
        help: 'Found in the Twilio Console → Account Info dashboard.' },
      { key: 'auth_token',  label: 'Auth Token',  type: 'password', required: true,
        help: 'Found next to the Account SID in Twilio Console → Account Info.' },
    ],
  },
  ...
  github: {
    ...
    setupSteps: [
      ...
      'Select the repo scope (and optionally issues, pull_requests); copy the generated token',
      'Paste the token, repository owner, and repository name below; optionally add a webhook secret',
    ],
    fields: [
      ...
      { key: 'repo',           label: 'Repository Name',  type: 'text', required: true,
        ...
        help: 'The repository name (without the owner prefix).' },
  ...
  signal: {
    ...
    docsSlug: 'signal',
    setupSteps: [
      'Install and run signal-cli-rest-api (github.com/bbernhard/signal-cli-rest-api) on a machine that stays powered on',
      'Register your phone number via the REST API: POST /v1/register/{number}',
      'Paste the REST API base URL and the registered phone number below',
    ],
    fields: [
      { key: 'api_url', label: 'signal-cli REST API URL', type: 'text',
        placeholder: 'http://localhost:8080', required: true,
        help: 'Run signal-cli-rest-api and point Qorven at its base URL.' },
      ...
      { key: 'use_websocket', label: 'Use WebSocket (receive events)', type: 'toggle',
        help: 'Enable to receive messages via WebSocket instead of polling. Recommended when the REST API is reachable.' },
    ],
  },
  ...
  zalo: {
    ...
    setupSteps: [
      ...
      'Complete the OA authorization flow to obtain a Refresh Token (the OA is derived automatically from the access token)',
    ],
    fields: [
  ...
  dingtalk: {
    ...
    docsSlug: 'dingtalk',
    setupSteps: [
      'Go to open.dingtalk.com → Create an application → copy App Key and App Secret (shown as Client ID / Client Secret in newer console versions)',
    ...
    fields: [
      { key: 'app_key',    label: 'App Key',    type: 'text',     required: true,
        help: 'DingTalk Open Platform → app credentials → App Key (called Client ID in newer console versions).' },
      { key: 'app_secret', label: 'App Secret', type: 'password', required: true,
        help: 'DingTalk Open Platform → app credentials → App Secret (called Client Secret in newer console versions).' },
  ...
  wecom: {
    ...
    fields: [
      ...
      { key: 'agent_secret', label: 'Agent Secret (Application Secret)', type: 'password', required: true,
      ...
      { key: 'encoding_key', label: 'EncodingAESKey', type: 'password',
  ...
  mattermost: {
    ...
    fields: [
      ...
      { key: 'team_id',        label: 'Team ID',        type: 'text',
        help: 'The team\'s ID from Mattermost — System Console → Teams, or the URL.' },
      { key: 'require_mention', label: 'Require @mention', type: 'toggle',
        help: 'Only respond when the bot is @mentioned in a channel.' },
    ],
  },
};
